...
- Логирует ошибки с входными параметрами
- Поддерживает вывод в консоль или файл (через параметр `filename`)
...
#### Тестами покрыты все основные модули проекта:
```
masks.py
processing.py
widget.py
...
При написании тестов используются фикстуры и параметризация, что позволяет проверять функции на различных входных данных без дублирования кода.
...
```
После выполнения отчёт будет доступен в [папке](htmlcov/index.html).
